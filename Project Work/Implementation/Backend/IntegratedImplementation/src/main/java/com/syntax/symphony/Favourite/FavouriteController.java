...
public class FavouriteController {
    ...
    @GetMapping ("/{id}/{carid}")
    public ResponseEntity<String> createFavourite(@PathVariable Long id, @PathVariable String carid) {
        System.out.println("Creating new favourite location");
        int created = userService.createFavourite(id, carid);
        switch (created) {
            case -1:
                return new ResponseEntity<String>("carpark id " + carid + " not found", HttpStatus.NOT_FOUND);
            case -2:
